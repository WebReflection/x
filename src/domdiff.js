  ...
  while (aStart < aEnd || bStart < bEnd) {
    ...
    // remove head or tail: fast path
      ...
      while (aStart < aEnd) {
        ...
        if (!map.has(a[aStart])) get(a[aStart], -1).remove();
        aStart++;
      }
    }
    // same node: fast path
    ...
    // map based fallback, "slow" path
      ...
      if (map.has(a[aStart])) {
        ...
        const idx = map.get(a[aStart]);
        // if it's not already processed, look on demand for the next LCS
        if (bStart < idx && idx < bEnd) {
          let i = aStart;
          // counts the amount of nodes that are the same in the future
          let seq = 1;
          while (++i < aEnd && i < bEnd && map.get(a[i]) === (idx + seq)) seq++;
          ...
          if (seq > (idx - bStart)) {
            ...
            while (bStart < idx) prev.push(get(b[bStart++], 1));
            node.before(...prev);
          }
